...
until_tick(23);assert(c.visual.direction_index==1)
until_tick(24);assert(c.visual.direction_index==0)
until_tick(48);assert(c.visual.direction_index==23 and storage.aiming_cannons[1].aim_state=='ELEVATING')
until_tick(107);assert(c.visual.elevation_index==0)
until_tick(108);assert(c.visual.elevation_index==1)
until_tick(168);assert(c.visual.elevation_index==2 and storage.aiming_cannons[1].aim_state=='SETTLING')
until_tick(227);assert(f.loaded_shots==2 and not next(storage.in_flight_shots))
until_tick(228)
local shot=storage.in_flight_shots[1]
assert(shot and shot.fire_tick==228 and shot.impact_tick==228+shot.flight_ticks)
...
until_tick(287);assert(storage.next_shot_id==2 and f.loaded_shots==1)
until_tick(288);assert(storage.in_flight_shots[2].fire_tick==288 and f.loaded_shots==0)
assert(c.visual.direction_index==last_pose[1] and c.visual.elevation_index==last_pose[2])
until_tick(888);assert(not next(storage.in_flight_shots) and not next(storage.countdown_by_tick))
...
assert(storage.in_flight_shots[4].fire_tick==300 and storage.in_flight_shots[4].flight_ticks==1800)
...
until_tick(24);assert(c.visual.direction_index==1)
until_tick(288);assert(c.visual.direction_index==12)
until_tick(348);assert(c.visual.elevation_index==3)
until_tick(528);assert(c.visual.elevation_index==0)
until_tick(588);assert(math.abs(storage.in_flight_shots[1].target_position.x)<1e-8)
