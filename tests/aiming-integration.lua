...
script.on_event(defines.events.on_tick,function(event)
  ...
  if first then
    ...
    if not storage.aiming_test then setup() else
      ...
      for i,phase in ipairs({'TRAVERSING','ELEVATING','SETTLING'}) do
        ...
        local pose=storage.cannons[t.ids[i]].visual
        check(pose.render.x_scale==(pose.direction_index>12 and -1 or 1),'reload mirror sign '..phase)
      end
  ...
  if tick==24 then check(a.visual.direction_index==0,'shortest path first step 01 to 00') end
  if tick==48 then
    check(a.visual.direction_index==23,'wrap 00 to 23')
    check(a.visual.render.x_scale==-1,'wrapped direction uses mirrored sprite')
  end
  if tick==60 then check(b.visual.elevation_index==1,'elevation single step') end
  ...
  if tick==180 then check(storage.in_flight_shots[2].fire_tick==180,'elevation plus settling deadline') end
  if tick==210 and game.planets.gleba then
  ...
  if tick==212 and game.planets.gleba then
  ...
  if tick==443 then check(storage.foundations[t.foundation_ids[1]].loaded_shots==2,'long aim keeps magazine until fire') end
  if tick==444 then
    local shot=storage.in_flight_shots[3]
    check(shot and shot.fire_tick==444 and shot.countdown.valid,'traverse elevate settle then fire')
  ...
  if tick==504 and t.planet then
    local shot=storage.in_flight_shots[4]
    check(shot and shot.fire_tick==504 and shot.flight_type=='interplanetary','actual planetary fire after settle')
    check(shot.flight_ticks==1800 and shot.impact_tick==2304,'planetary flight excludes aiming time')
  end
  if tick==2305 then
