...
$run = Join-Path $repo ('.codex-test-output/stage3-' + [guid]::NewGuid().ToString('N'))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'bootstrap.lua') -Destination (Join-Path $mod 'control.lua')
  ...
  try {
    while (-not $process.HasExited) {
      ...
      if ((Test-Path -LiteralPath $currentLog) -and
        (Select-String -LiteralPath $currentLog -Pattern 'STAGE3 ALL PASSED' -Quiet)) {
        $passed = $true
...
Copy-Item -LiteralPath $save -Destination (Join-Path $run 'saves/stage3-flight.zip')
...
foreach ($phase in @('initial', 'reload')) {
  ...
  if ($phase -eq 'reload') { $save = Join-Path $run 'saves/stage3-flight.zip' }
  ...
  if (-not (Select-String -LiteralPath $log -Pattern 'STAGE3 ALL PASSED' -Quiet)) {
    throw "Stage $phase failed: $log"
  }
  if ($phase -eq 'reload' -and -not (Select-String -LiteralPath $log -Pattern 'STAGE3 RELOAD CONFIRMED' -Quiet)) {
    throw "Reload state was not verified: $log"
  }
  Select-String -LiteralPath $log -Pattern 'STAGE3 (PASS|ALL|RELOAD|IN-FLIGHT)' | ForEach-Object { $_.Line }
}
